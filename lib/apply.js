 ...
 * POST 말고는 안 받습니다.
 *
 * 옛 Pages Function 시절의 주석을 그대로 옮깁니다 — *「onRequestPost 만
 * 두면 GET 요청이 이 함수를 그냥 지나쳐 정적 파일 쪽으로 흘러가고, Pages 가
 * index.html 을 200 으로 내줍니다. /apply 가 홈페이지의 사본이 되어
 * 검색엔진에 중복으로 잡힙니다.」*
 *
 * 지금은 Next 의 라우트라 GET 이 흘러가지는 않습니다만, **405 를 우리 말로
 * 돌려주는 것**은 그대로 값이 있습니다.
 */
export function methodNotAllowed(request) {
  // 몸통을 읽기 전이라 헤더가 가진 전부입니다.
  const t = pick(null, request.headers.get('accept-language'));
  return json(405, { ok: false, error: t.methodNotAllowed });
}

/**
 * 신청 한 건을 받아 메일 두 통을 보냅니다.
 *
 * **로직은 Pages Function 시절과 한 줄도 다르지 않습니다.** 바뀐 것은
 * 부르는 껍데기뿐입니다(`app/apply/route.ts`) — `context` 대신 `request` 와
 * `env` 를 직접 받습니다.
 */
