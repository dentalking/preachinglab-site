...
export type Landing = {
  ...
  nav: {
    ...
    apply?: string;
    /**
     * 이미 쓰고 계신 분이 들어가는 문. **네 말 모두에 있습니다.**
     *
     * 오래 이 문이 화면 어디에도 없었습니다. 넘김은 서버에 있었는데
     * (`public/_redirects` 의 `/my`) 그리로 가는 링크를 아무도 안
     * 그려서, **주소를 직접 치셔야만** 들어가졌습니다. 이미 쓰시는
     * 목사님이 랜딩에 오시면 보이는 것이 「파일럿 신청하기」뿐이었습니다.
     *
     * **`/my` 로 씁니다.** 절대 주소(`my.preachinglab.cloud`)를 박지
     * 않습니다 — 목사님께 알려드리는 주소는 `preachinglab.cloud` 하나여야
     * 하고, 넘김이 그것을 받습니다.
     */
    mine: string;
  };
